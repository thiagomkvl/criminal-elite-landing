...
type ImageItem = BaseItem & {
  type: "image";
  src: string;          // pode ser import do Vite (ex.: import img from '@/assets/...') ou /public
};
...
type Props = {
  ...
  className?: string;
  autoPlayMs?: number | null; // ex.: 5000 para autoplay. null desativa
};
...
const PortfolioCarousel: React.FC<Props> = ({
  ...
  return (
    <div
      className={`relative w-full overflow-hidden rounded-2xl bg-[#111]/40 ring-1 ring-white/5 ${heightClass} ${className}`}
    >
      {/* faixa para garantir altura e empilhamento */}
